...
class Reader(DataReader):
    ...
    def _get_charge_exchange(
        ...
        results = {}
        results["length"] = np.random.randint(4, 20)
        results["dt"] = np.random.uniform(0.001, 1.0)
        results["times"] = np.arange(TSTART, TEND, results["dt"])
        results["texp"] = np.full_like(results["times"], results["dt"])
        nt = len(results["times"])
    ...
    def _get_radiation(
        ...
        results["length"] = {}
        results["length"]["v"] = length
        results["length"]["h"] = length
