...
class TestPlasmaCaching:
    def setup_method(self):
        ...
        self.property_getters = ["Fz", "Ion_density", "Lz_tot", "Meanz", "Zeff", "Total_radiation",
                                 "Electron_pressure", "Thermal_pressure",]
        self.properties = ["fz", "ion_density", "lz_tot", "meanz", "zeff", "total_radiation",
                           "electron_pressure", "thermal_pressure",]
    ...
    def test_property_values_match_operator_calls(self):
        ...
        for property in self.properties:
            if type(getattr(self.plasma, property)) is not dict:  # fz should really not be a dict!
                assert (getattr(self.plasma, property) == getattr(self.plasma, f"calc_{property}")()).all()
