...
def indica2bbnbi(time, equil, el_dens, el_temp, ion_dens, ion_temp, beam):
    ...
    fpsi=interp.interp2d(equil.psi["R"], equil.psi["z"], equil.psi[it,:,:],
                         kind='cubic')
    B2D["psiaxis"] = fpsi(equil.rmag[it],equil.zmag[it])
    B2D["psisepx"] = fpsi(equil.rbnd[it,0],equil.zbnd[it,0])
    psiv = (equil.f["rho_poloidal"] * (B2D["psisepx"] - B2D["psiaxis"])
            + B2D["psiaxis"])
    f = interp.interp1d(psiv, equil.f[it,:], kind='linear', bounds_error=False,
    ...
    B_2D.write_hdf5(fn,
                    B2D["Rmin"], B2D["Rmax"], B2D["nR"],
                    B2D["zmin"], B2D["zmax"], B2D["nz"],
                    B2D["axisR"], B2D["axisz"],
                    B2D["psiRz"].T, B2D["psiaxis"], B2D["psisepx"],
                    B2D["B_R"], B2D["B_phi"], B2D["B_z"])
    ...
    for i in range(len(dist["r"])):
        for j in range(len(dist["phi"])):
            for k in range(len(dist["z"])):
               hist[i,j,k] /= (2 * np.pi
                               * (dist["r_edges"][i]+dist["r_edges"][i+1])/2
                               * (dist["r"][1]-dist["r"][0])
                               * (dist["z"][1]-dist["z"][0]))
