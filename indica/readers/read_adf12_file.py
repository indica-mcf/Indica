...
# Get ADF12 file
qener, qtiev, qdensi, qzeff, qefref = adas_reader.get_adf12(
    "H", "0", "C", "6", "cx", "99"
)
...
lnq += np.interp(
    np.log(ne_cm3), np.log(qdensi[qdensi.dims[0]].data), np.log(qdensi.data)
)
...
for i in range(len(ne_cm3_vec)):
    for j in range(len(Ti_eV_vec)):
        lnq[i, j] += np.interp(
            np.log(Ti_eV_vec[j]), np.log(qtiev[qtiev.dims[0]].data), np.log(qtiev.data)
        )
        lnq[i, j] += np.interp(
            np.log(ne_cm3_vec[i]),
            np.log(qdensi[qdensi.dims[0]].data),
            np.log(qdensi.data),
        )
lnq = np.exp(lnq) * 1e-6  # m^3 / s
