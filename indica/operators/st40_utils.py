...
def extract_hda_plasma(
    ...
    plasma = {
        "data_source": "HDA " + str(shot) + " RUN" + str(run),
        "time": time,
        "mask": mask,
        "dene": dene,
        "denn": denn,
...
def Tait_Bryan_rotate(point_xyz, alpha, beta, gamma):
    ...
    alpha_yaw = np.array(
        (
            np.array((np.cos(alpha), -1.0 * np.sin(alpha), 0)),
    ...
    beta_pitch = np.array(
        (
            ...
            np.array((0, 1, 0)),
            np.array((-1.0 * np.sin(beta), 0, np.cos(beta))),
        )
    )
    gamma_roll = np.array(
        (
            np.array((1, 0, 0)),
            np.array((0, np.cos(gamma), -1.0 * np.sin(gamma))),
...
def gaussian(x, offset, amp, std, cwl):
    ...
    return offset + amp / (std * np.sqrt(np.pi)) * np.exp(
        -1 * (x - cwl) ** 2 / (std * std)
    )
...
def get_v_tor_v_pol(los_origin, los_pini_intersect, del_lambda, cwl):
    ...
    gamma = np.arccos(
        np.dot(los_vec, tan_vec)
        / (np.sqrt(np.dot(los_vec, los_vec)) * np.sqrt(np.dot(tan_vec, tan_vec)))
    )
...
def read_cxs_spec_geometry(
    shot, origin=[107.7264, -36.442098, 0], spec_name="Princeton"
):
...
def read_adf12_qef(adf12file, transition="8-7", level="n1"):
    ...
    qef_dict[transition]["n1"] = {
        "qefref": 9.91e-09,
        "parmref": np.array((4.00e04, 5.00e03, 2.50e13, 2.00e00, 3.00e00)),
        "ener": np.array(
            (
                1.00e03,
                1.50e03,
...
def create_st40_beam_grid(
    beam, plot_bgrid=False, ax=None, delta_src=0.0, delta_ang=0.0
):
    ...
    if ax:
        for beam in nbi_list:
            ax.scatter(
                beam["src"][0], beam["src"][1], beam["src"][2], marker="x", color="k"
            )
            pini_len = 1000
            ...
            if chord == key:
                if plot2d:
                    ax.plot(
                        [
                            item["origin"][0],
                            item["origin"][0] + item["diruvec"][0] * los_len,
                        ],
                        [
                            item["origin"][1],
                            item["origin"][1] + item["diruvec"][1] * los_len,
                        ],
                        color="r",
                    )
                else:
                    ax.plot3D(
                        ...
                        zs=[
                            item["origin"][2],
                            item["origin"][2] + item["diruvec"][2] * los_len,
                        ],
                        color="r",
                    )
