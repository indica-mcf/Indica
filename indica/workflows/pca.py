...
class PCAProcess:
    ...
    def __post_init__(self):
        ...
        for profile_name in self.gaussian_profiles.keys():
            ...
            self.pca_weights[profile_name], self.pca_fits[profile_name] = self.pca_fit(
                self.gaussian_profiles[profile_name])
            self.prior_fits[profile_name] = self.fit_priors(self.pca_weights[profile_name])
            name = "/".join([f"{profile_name}.basis_{i + 1}" for i in range(self.ncomps)])

            self.conditional_priors[name] = self.prior_fits[profile_name]
            self.reconstructed_profiles[profile_name] = self._reconstruct_profile(
            ...
            self.projected_priors[profile_name], self.projected_prior_meshgrid[profile_name] = \
                self.project_priors(self.prior_fits[profile_name], self.pca_weights[profile_name])
            self.KDE_priors[profile_name] = self.fit_KDE_prior(self.prior_fits[profile_name], )
        self.basis_priors = flatdict.FlatDict(self.KDE_priors, ".")

    def pca_fit(self, profiles: np.ndarray, verbose=True) -> Tuple[np.ndarray, PCA]:
        pca = PCA(n_components=self.ncomps)
        pca.fit(profiles)
    ...
    def fit_priors(self, pca_profiles: np.ndarray, prior_values=None, ) -> LinearNDInterpolator:
    ...
    def project_priors(self, prior_fit, pca_profiles) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def fit_KDE_prior(self, linear_fit: LinearNDInterpolator, size=100000) -> gaussian_kde:
    ...
    def _reconstruct_profile(self, basis_function: np.ndarray, bias: np.ndarray, weights: np.ndarray, ):
        return np.dot(weights, basis_function) + bias

    def _plot_principle_comps(self, pca, profile_name):
        plt.figure()
    ...
    def _plot_profile_fits(self, profiles: np.ndarray, projected_profiles: np.ndarray, profile_name, nsamples: int = 100):
    ...
    def _plot_projected_priors(self, Z, meshgrid, pca_profiles, profile_name):
        if Z.ndim == 2:
            plt.figure()
    ...
    def _plot_KDE_comparison(self, kernels: Dict[str, gaussian_kde], prior: LinearNDInterpolator, size=1000):
    ...
    def plot_profile(self, profile_name: str):
        ...
        self._plot_profile_fits(self.gaussian_profiles[profile_name], self.reconstructed_profiles[profile_name], profile_name)
        self._plot_principle_comps(self.pca_fits[profile_name], profile_name)
        self._plot_projected_priors(self.projected_priors[profile_name],
                                    self.projected_prior_meshgrid[profile_name],
                                    self.pca_weights[profile_name],
                                    profile_name)
        self._plot_KDE_comparison(self.KDE_priors[profile_name], self.prior_fits[profile_name])
