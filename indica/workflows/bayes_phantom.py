...
from indica.readers.read_st40 import ReadST40  # ST40data


def plot_bayes_phantom(
    ...
    plt.plot(
        np.arange(0, autocorr.__len__())[mask], autocorr[mask], label="average tau"
    )
    plt.legend()
    ...
    if phantom_profiles:
        phantom_profiles["electron_density"].plot(
            label="phantom_profile", linestyle="--", color="black"
        )
    plt.legend()
    ...
    if phantom_profiles:
        phantom_profiles["electron_temperature"].plot(
            label="Te, phantom_profile", linestyle="--", color="black"
        )
    ...
    if phantom_profiles:
        phantom_profiles["ion_temperature"].plot(
            label="Ti, phantom_profile", linestyle="-.", color="black"
        )
    plt.legend()
    plt.ylabel("temperature (eV)")
...
def initialise_diag_data(plasma, pulse, tstart=0.02, tend=0.10, dt=0.01):
    read_data = ReadST40(pulse, tstart - dt * 4, tend + dt * 4)
    read_data(instruments=["xrcs", "smmh1", "cxff_pi"])
...
if __name__ == "__main__":
    ...
    autocorr = sample_with_autocorr(
        sampler, start_points, iterations=1000, auto_sample=10
    )
