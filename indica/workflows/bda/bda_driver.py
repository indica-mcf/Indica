...
class BDADriver:
    ...
    def _build_result_dict(
        ...
        result["MODEL_DATA"]["SAMPLE_IDX"] = np.arange(0, self.opt_samples["post_sample"].shape[1])

        result["PHANTOMS"] = {
            "FLAG": self.plasma_profiler.phantom,
            "PSI_NORM": {
                "RHOP": self.plasma_profiler.plasma.rho,
                "NE": self.plasma_profiler.phantom_profiles["PSI_NORM"]["electron_density"],
                "TE": self.plasma_profiler.phantom_profiles["PSI_NORM"]["electron_temperature"],
                "TI": self.plasma_profiler.phantom_profiles["PSI_NORM"]["ion_temperature"],
                "NI": self.plasma_profiler.phantom_profiles["PSI_NORM"]["ion_density"],
                "NNEUTR": self.plasma_profiler.phantom_profiles["PSI_NORM"]["neutral_density"],
                "NFAST": self.plasma_profiler.phantom_profiles["PSI_NORM"]["fast_density"],
                "ZEFF": self.plasma_profiler.phantom_profiles["PSI_NORM"]["zeff"].sum(dim="element"),
                "MEANZ": self.plasma_profiler.phantom_profiles["PSI_NORM"]["meanz"],
                "PTH": self.plasma_profiler.phantom_profiles["PSI_NORM"]["pressure_th"],
                "PFAST": self.plasma_profiler.phantom_profiles["PSI_NORM"]["pressure_fast"],
                "P": self.plasma_profiler.phantom_profiles["PSI_NORM"]["pressure_tot"],
                "VTOR": self.plasma_profiler.phantom_profiles["PSI_NORM"]["toroidal_rotation"],
            },
            "R_MIDPLANE": {
                "RPOS": self.plasma_profiler.plasma.R_midplane,
                "ZPOS": self.plasma_profiler.plasma.z_midplane,
                "NE": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["electron_density"],
                "TE": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["electron_temperature"],
                "TI": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["ion_temperature"],
                "NI": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["ion_density"],
                "NNEUTR": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["neutral_density"],
                "NFAST": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["fast_density"],
                "ZEFF": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["zeff"].sum(dim="element"),
                "MEANZ": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["meanz"],
                "PTH": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["pressure_th"],
                "PFAST": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["pressure_fast"],
                "P": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["pressure_tot"],
                "VTOR": self.plasma_profiler.phantom_profiles["R_MIDPLANE"]["toroidal_rotation"],
            },
        }
        ...
        result["PROFILES"] = {
            "PSI_NORM": {
                "RHOP": self.plasma_profiler.plasma.rho,
                "RHOT": self.plasma_profiler.plasma.equilibrium.rhotor.interp(t=self.plasma_profiler.plasma.t),
            ...
            "R_MIDPLANE": {
                ...
                "VTOR": self.midplane_blobs["toroidal_rotation"].median(dim="sample_idx"),
                "VTOR_ERR": self.midplane_blobs["toroidal_rotation"].std(dim="sample_idx"),
            },
        }
        ...
        result["GLOBAL"] = {
            ...
            "TI0": self.blobs["ion_temperature"].sel(rho_poloidal=0, method="nearest").median(dim="sample_idx"),
            "TE0": self.blobs["electron_temperature"].sel(rho_poloidal=0, method="nearest").median(dim="sample_idx"),
            "NE0": self.blobs["electron_density"].sel(rho_poloidal=0, method="nearest").median(dim="sample_idx"),
            ...
            "ZEFF_AVG": self.midplane_blobs["zeff"].sum(dim="element").median(dim="sample_idx").mean(dim="R"),
            "NNEUTR0": self.blobs["neutral_density"].sel(rho_poloidal=0, method="nearest").median(dim="sample_idx"),
            "NNEUTRB": self.blobs["neutral_density"].sel(rho_poloidal=1, method="nearest").median(dim="sample_idx"),
            "TI0_ERR": self.blobs["ion_temperature"].sel(rho_poloidal=0, method="nearest").std(dim="sample_idx"),
            "TE0_ERR": self.blobs["electron_temperature"].sel(rho_poloidal=0, method="nearest").std(dim="sample_idx"),
            "NE0_ERR": self.blobs["electron_density"].sel(rho_poloidal=0, method="nearest").std(dim="sample_idx"),
            "NI0_ERR": self.blobs["ion_density"].sel(rho_poloidal=0, method="nearest").std(dim="sample_idx"),
            "WTH_ERR": self.blobs["wth"].std(dim="sample_idx"),
            "WP_ERR": self.blobs["wp"].std(dim="sample_idx"),
            "ZEFF_AVG_ERR": self.midplane_blobs["zeff"].sum(dim="element").std(dim="sample_idx").mean(dim="R"),
            "NNEUTR0_ERR": self.blobs["neutral_density"].sel(rho_poloidal=0, method="nearest").std(dim="sample_idx"),
            "NNEUTRB_ERR": self.blobs["neutral_density"].sel(rho_poloidal=1, method="nearest").std(dim="sample_idx"),
            "VTOR0": self.blobs["toroidal_rotation"].sel(rho_poloidal=0, method="nearest").median(dim="sample_idx"),
            "VTOR0_ERR": self.blobs["toroidal_rotation"].sel(rho_poloidal=0, method="nearest").std(dim="sample_idx"),
        }
        return result
    ...
    def __call__(
        ...
        # TODO: fix problem with manually selecting time index and having floating point precision errors
        ...
        self.blobs = blobs
        self.midplane_blobs = self.plasma_profiler.map_plasma_profiles_to_midplane(blobs)
