...
class SXR_tomography():
    ...
    def __init__(self, data, ):         
        ...
        self.eq = data['rho_equil']
 
        ##### Parameters
        self.nlos, self.los_len = data['R'].shape
        ...
        #IMPACT PARAMETERS
        self.impact_parameters = data['impact_parameters']
        
        self.debug = data['debug']

        #load data
        self.data = data['brightness'] #W/m^2 ??
    ...
    def geom_matrix(self ):
        ...
        self.x_inter = self.x[:,None]+np.linspace(-1,1,self.nvirt)*(1-1/self.nvirt)/2
        ...
        for it in range(nt):
            ...
            LOS_rho = RectBivariateSpline(self.eq['R'], self.eq['z'], self.eq['rho'][it].T).ev(R,z)
                ...
                for iv in range(self.nvirt):
                    ...
                    for i in range(len(i_extrema)-1):
                        ...
                        Ledge = interp1d(rho_cut[monotone_ind], L[monotone_ind],
                                        bounds_error=False, fill_value = 0)(self.rho_grid_edges)
                        ...
                        dLmat[it, ilos,iv, i_tg_in:i_tg_out] += np.abs(Ledge[i_tg_in:i_tg_out]-Ledge[i_tg_in+1:i_tg_out+1])
                        
                        #add the turning point singularity
                        if  rho_cut[monotone_ind][-1]-rho_cut[monotone_ind][0] > 0: # if L increases with rho
                            if i_tg_in > 0:
                                dLmat[it,ilos,iv,i_tg_in-1] +=  Ledge[i_tg_in]-L_turn
                            if i_tg_out < self.nr: #this should happen just in the last step
                                L_turn = Ledge[i_tg_out]
                                assert i < len(i_extrema)-1
                        else:   # if L decreases with rho
                            if i_tg_out < self.nr: #this should happen just in the first step
                                dLmat[it,ilos,iv,i_tg_out] +=  Ledge[i_tg_out]-L_turn
        ...
        self.dLmat_interleaved = convolve(dLmat_interleaved,
                            np.ones((1,self.nvirt,1))/self.nvirt, mode='nearest')
        
        rho_tg_interleaved = rho_tg.reshape(nt, -1)
        self.rho_tg_interleaved = convolve(rho_tg_interleaved,
                            np.ones((1,self.nvirt))/self.nvirt, mode='nearest')
    ...
    def calc_tomo(self, reg_level = 0.8, nfisher = 3, eps = 1e-2, optim_regul=False):
        ...
        teq_ind = np.int_(interp1d(self.eq['t'],np.arange(len(self.eq['t'])),
                           kind='nearest', fill_value='extrapolate')(self.tvec))
        ...
        for it,teq in enumerate(self.eq['t']):
            ...
            for ifisher in range(nfisher):
                ...
                H = solve_banded((1,1),DTW,T.T, overwrite_ab=False,check_finite=False)
                ...
                y = solve_banded((1,1),WD,y, overwrite_ab=False,
            ...
            V = np.dot(H,U/S)  
            V = solve_banded((1,1),WD,V, overwrite_ab=False,
                            overwrite_b=True,check_finite=False)
            ...
            self.chi2[t_ind] = np.sum((d-fit)**2,1)/np.size(fit,1) #TODO jen ty funkcni ch

            self.emiss[t_ind] = y
            self.emiss_err[t_ind] = np.sqrt(np.dot(V**2,(w/S)**2)*np.maximum(1,self.chi2[t_ind,None]))
    ...
    #reg_level_max = .4
        ...
        slide_reg = Slider(ax_reg, 'Regularisation:', 0,1,
                    valinit=self.reg_level_guess , valstep=0.001, valfmt='%1.3f')

        ax_time = plt.axes([0.2, 0.07, 0.65, 0.03], facecolor='y')
        slide_time = Slider(ax_time, 'Time [s]:', self.tvec[0], self.tvec[-1],
                    valinit=self.tvec[0], valstep=0.001, valfmt='%1.3f')

        r = self.rho_grid_centers
        f.subplots_adjust(bottom=.2)
 
        tomo_var = ax[0].fill_between(r, r*0, r*0, alpha=.5, facecolor='b', edgecolor='None')
        tomo_mean, = ax[0].plot([],[], lw=2)
        
        
        errorbar = ax[1].errorbar(0,np.nan,0,  capsize = 4
                                ,c='g',marker='o',ls='none')
        ...
        ax[0].set_ylim(0, np.nanmax(self.emiss)*1.2/1e3)
        ...
        cont = ax[2].contour(self.eq['R'],self.eq['z'],
                             self.eq['rho'][0], cvals,colors='k')
        ...
        def update(reg=None,time=None):
            global cont
            if reg is not None:
                self.calc_tomo(reg_level = reg**.5,  nfisher = 3, eps = 1e-5)
            ...
            update_fill_between(tomo_var,r,self.emiss[it]/1e3-self.emiss_err[it]/1e3,
                                self.emiss[it]/1e3+self.emiss_err[it]/1e3,-np.inf,np.inf)
            ...
            cont = ax[2].contour(self.eq['R'],self.eq['z'],self.eq['rho'][it_eq], cvals,colors='k')

  
            title.set_text('  $\chi^2/nDoF$ = %.1f  $\gamma$ = %.2f'%( self.chi2[it], self.gamma[it]))
            f.canvas.draw_idle()
    ...
    def save(self,args):
        np.savez_compressed('Reconstruction',backprojection= self.backprojection,brightness=self.data, 
                            brightness_err = self.err,time=self.tvec, grid=self.rho_grid_centers,
                            gamma =self.gamma,chi2=self.chi2, 
                            emiss=self.emiss.astype('single') ,emiss_err = self.emiss_err.astype('single') )
    ...
    def __call__(self):
        ...
        def get_emissivity_2D(data,eq_data):
            ...
            for it,t in enumerate(data['t']):
                ...
                sel_ind = np.where(np.abs(eq_data['t']-t)==np.nanmin(np.abs(eq_data['t']-t)))[0][0]
                #SELECTING RHO
                sel_rho = rho[sel_ind,:,:].T
                #2D VALUE OF EMISSIVITY
                emiss[it,:,:] = interp1d(rho_1D[it,:],emiss_1D[it,:],bounds_error=False,fill_value=0)(sel_rho)
            #RETURN DATA
            return_data = dict(
                R = R,
                z = z,
        ...
        return_data = dict(
            ...
            profile = dict(
                ...
                rho_poloidal = np.repeat(np.array([self.rho_grid_centers]),len(self.tvec),axis=0)
                ),                
            )
        #EMISSIVITY 2D
        ...
        return_data['back_integral']['chi2'] = np.sqrt(np.nansum(((data_exp-data_the)**2) / (data_exp**2),axis=1))
